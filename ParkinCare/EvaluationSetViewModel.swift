...
/*class EvaluationSetViewModel : EvaluationTableViewModel{
 ...
 request.sortDescriptors = [NSSortDescriptor(key:#keyPath(Evaluation.lastname),ascending:true),NSSortDescriptor(key:#keyPath(Evaluation.firstname),ascending:true)]
 ...
 /// update birth date of Evaluation
 ///
 /// - Parameters:
 ///   - indexPath: (section,row) of Evaluation we want to update the birth date
 ///   - date: birth date
 /*public func updateBirthDate(atIndexPath indexPath: IndexPath, withDate date: Date){
 let Evaluation = self.evaluationsFetched.object(at: indexPath)
 Evaluation.birthdate = date
 self.delegate?.EvaluationUpdated(at: indexPath)
 }*/
